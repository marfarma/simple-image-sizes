var regenerate={post_types:'',thumbnails:'',list:'',cur:0,percent:'',getThumbnails:function(){var self=this;var inputs=jQuery('input.thumbnails:checked');if(inputs.length!=jQuery('input.thumbnails[type="checkbox"]').length){inputs.each(function(i){self.thumbnails+='&thumbnails[]='+jQuery(this).val();});}},getPostTypes:function(){var self=this;var inputs=jQuery('input.post_types:checked');if(inputs.length!=jQuery('input.post_types[type="checkbox"]').length){inputs.each(function(){self.post_types+='&post_types[]='+jQuery(this).val();});}},setMessage:function(msg){jQuery("#regenerate_message").html(msg).addClass('updated').addClass('fade').show();this.refreshProgressBar();},refreshProgressBar:function(){jQuery(".progress").progressbar();},startRegenerating:function(){var self=this;jQuery.ajax({url:sis.ajaxUrl,type:"POST",data:"action=ajax_thumbnail_rebuild&do=getlist"+self.post_types,beforeSend:function(){jQuery("#ajax_thumbnail_rebuild").attr("disabled",true);self.setMessage("<p>"+sis.reading+"</p>");self.getThumbnails();self.getPostTypes();},success:function(r){self.list=eval(r);self.curr=0;jQuery('.progress').show();self.regenItem();}});},regenItem:function(){var self=this;if(!this.list){this.reInit();this.setMessage(sis.noMedia);return false;}
